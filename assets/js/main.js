...
then(
    (data) => {
    const products = data.productos.slice(0, 20);
    ...
    let filterProducts = data.productos;
    const txt_search = document.querySelector('#txt-search');

    document.querySelector('#btn-search').addEventListener('click', () => {
        let res = filterProducts.filter(product => {
            return product.nombre.includes(txt_search.value);
        });
        if(res.length > 0) {
            const productContainer = document.querySelector('#product-container');
        ...
        res.forEach(item => {
            const datosProducto = {
                ...
                imagen_frontal: item.imagen_frontal,
                imagen_lateral : item.imagen_lateral,
                imagen_extra: item.imagen_general,
            }
